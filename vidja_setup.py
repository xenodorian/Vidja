...
def extract_7z_or_zip(archive: Path, dest: Path) -> None:
    ...
    r = subprocess.run([seven, "x", str(archive), f"-o{dest}", "-y", "-bsp1"])
    if r.returncode != 0:
...
def repair_portable_comfyui(root: Path) -> None:
    """If extract left ComfyUI without main.py, copy source from the Vidja repo root."""
    main_py = root / "ComfyUI" / "main.py"
    if main_py.is_file():
        return

    log(f"WARNING: {main_py} is missing after extract — repairing from Vidja source")
    ...
    preserved_models = comfy_dir / "models"
    temp_models = None
    if preserved_models.is_dir():
        temp_models = ROOT / "_vidja_models_backup"
        if temp_models.exists():
            shutil.rmtree(temp_models, ignore_errors=True)
        shutil.move(str(preserved_models), str(temp_models))
        log(f"Preserved existing models at {temp_models}")

    copied = 0
    for item in ROOT.iterdir():
        if item.name in skip_names or item.name.startswith("Vidja_"):
            continue
    ...
    if temp_models and temp_models.is_dir():
        dest_models = comfy_dir / "models"
        if dest_models.exists():
            for p in temp_models.rglob("*"):
                if p.is_file():
                    rel = p.relative_to(temp_models)
            ...
            shutil.move(str(temp_models), str(dest_models))

    if not main_py.is_file():
        if (ROOT / "main.py").is_file():
            shutil.copy2(ROOT / "main.py", main_py)
        else:
            die(
                f"Still missing {main_py} after repair. "
                "Re-extract the portable or re-clone the Vidja repo so main.py is present."
            )
    log(f"Repaired portable ComfyUI ({copied} items). main.py present: {main_py.is_file()}")
